...
\item In the absence of stimulation, there is a baseline oscillation of the anterior body (in our model: $\Theta_b$). The {\it amplitude} of each of these alternating head-casts is  continuously modulated by the stimulus perceived. %for a given state of the oscillator.
...
% With a negative gain, a positive $\nabla s$ (i.e. an increase in concentration perceived) on one step would lead to a decrease in turning away at the next step (up to 0 degree), while a negative $\nabla s$ (i.e. a decrease in concentration perceived) will lead to an increase in turning away  on the next step (up to 180 degrees). While a negative gain mediates attraction, a positive gain (i.e. positive and negative $\nabla s$ lead to increase and decrease in turning respectively) mediates repulsion. 
\end{itemize}
