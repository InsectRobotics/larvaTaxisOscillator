...
%\todo{BW: not clear to me why these are good choices for citations to support this point. Should mention 'breakthrough' methods papers...} 
...
% The ‘sampling’ behaviour and the goal-directed action are not separable.
 % 
%  \todo{BW: but note, that is not a claim made in the `state transition' models either - e.g. in Alex's model, the turn directions are random, and sharp increases in concentration lead to transitions back to running. So I don't think this belongs here.} KL : Good point, removed the laterized signal. 
